...
# File & dictionary management
    ...
    # This filters out existing result file. But will also print "Failed" return
    if cal_mode == "draw_xrd":
        for path, dirs, files in os.walk(out_dir):
            for file in files:
                if file == "cod_{}.txt".format(re.split(r"[.]", cif_file)[0]):
                    # clear_output(wait = True)
                    return "Cancelled: repeated CIF"
...
# If no cell parameters, we return failed
    if cell_a == 0 or cell_b == 0 or cell_c == 0:
        return "Failed: Cell length not assigned (a/b/c)"
    if cell_alpha == 0 or cell_beta == 0 or cell_gamma == 0:
        return "Failed: Cell angle not assigned (alpha/beta/gamma)"
    if cell_alpha >= 180 or cell_beta >= 180 or cell_gamma >= 180:
        return "Failed: Cell angle too large (>180) (alpha/beta/gamma)"
    ...
    for count_symm_atom, line in enumerate(cif_content_lines[atom_start_line - 1: atom_end_line]):
        ...
        # This error case happens when atom label is like "ALM"
            ...
            return f"Failed: Can not match neutrual atom type {symm_atom_type}"
    ...
    if cal_mode == "draw cell":
        print("return cif_info")
        return np.unique(symm_atom_info[:, 0].T), space_group
    ...
    if cal_mode == "draw_xrd" or cal_mode == "draw_cell":
        ...
        if True and num_symm_atom*num_symm_op > num_symm_atom_count:
...
#         print(cell_a, cell_b, cell_c, cell_alpha, cell_beta, cell_gamma)
        ...
        b = cell_b
        c = cell_c
        alpha = cell_alpha * np.pi/180
        beta  = cell_beta * np.pi/180
        gamma = cell_gamma * np.pi/180
        v = 0
...
#         print(struc)
        ...
        #print(f_hkl_sum.shape[:], f_hkl.shape[:])
    ...
    # After having a pure xy data, we merge their intensites if they are in one position
        ...
        if False:
            new_filename = "xy_cod_{}.txt".format(re.split(r"[.]", cif_file)[0])
...
#multiprocess of peak shape function
        ...
        new_filename = "cod_{}.txt".format(re.split(r"[.]", cif_file)[0])
        new_file= open("{}/{}".format(out_dir, new_filename), "w+")
        #np.savetxt(new_file, pattern, delimiter=',')
